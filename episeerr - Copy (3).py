...
def handle_sonarr_webhook():
    """Handle Sonarr webhook for potential tag-based filtering."""
    ...
    try:
        ...
        # Only process if it's a Grab event (case-sensitive)
        ...
        if not series or not episodes:
            logger.error("No series or episodes in payload")
            return jsonify({"error": "Incomplete payload"}), 400

        tvdb_id = series.get('TvdbId')
        if not tvdb_id:
            logger.error("No TVDB ID found")
            return jsonify({"error": "No TVDB ID"}), 400

        # Get full series info to check for episode tags
        headers = get_sonarr_headers()
        full_series_info = get_series_by_id(series.get('Id'), headers)
        
        if not full_series_info:
            logger.error(f"Could not retrieve full series info for TVDB ID {tvdb_id}")
            return jsonify({"error": "Could not retrieve series info"}), 500

        # Get episode numbers to monitor from tags
        episodes_to_monitor = get_episode_numbers_from_tags(full_series_info, headers)
        
        if not episodes_to_monitor:
            logger.warning(f"No episode tags found for series {series.get('Title')}")
            return jsonify({"message": "No episode tags found"}), 200

        # Extract the season of the grabbed episodes
        grabbed_season = episodes[0].get('SeasonNumber')
        
        # Proceed with tag-based filtering
        success = wait_and_setup_series(str(tvdb_id), grabbed_season)
